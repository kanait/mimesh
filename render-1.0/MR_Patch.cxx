...
MR_Patch::MR_Patch(std::vector<double>& src, std::vector<double>& trg,
                   int mesh_res) {
  count = 0;
  _mesh_res = mesh_res;
...
void MR_Patch::CreateIndex(int res)  //:
{
...
void MR_Patch::MakeVTree(int myself, int p0, int p1, int lv)  //:
{
  if (!_vec[myself].IsStore()) {
    dif_table[lv][ic[lv]] = myself;
    ic[lv]++;
    dif_table[lv][ic[lv]] = p0;
    ic[lv]++;
    dif_table[lv][ic[lv]] = p1;
    ic[lv]++;
    _vec[myself].SetParent(&_vec[p0], &_vec[p1]);
  }
  //  cout<<p0<<" "<<p1<<" \n";
}
MR_Mesh* MR_Patch::QTree(MR_Mesh* _p_parent, int x0, int y0, int x1, int y1,
                         int x2, int y2, int lv)  //:
{
  // Create patch
  ...
  MakeVTree(cx0 + idx_table[cy0], x0 + idx_table[y0], x1 + idx_table[y1],
            lv + 1);
  MakeVTree(cx1 + idx_table[cy1], x1 + idx_table[y1], x2 + idx_table[y2],
            lv + 1);
  MakeVTree(cx2 + idx_table[cy2], x2 + idx_table[y2], x0 + idx_table[y0],
            lv + 1);
...
void MR_Patch::SetParam(std::vector<double> d) { param = d; }

void MR_Patch::Set_Resolution(int res) { Create_Indexes(res, &index); }
